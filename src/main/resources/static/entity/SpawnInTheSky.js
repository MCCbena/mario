...
entityNBT        String    召喚するエンティティのNBTを指定します。
...
class SpawnInTheSky extends Entity{
    ...
    entityContactEvent(e) {
        if(e.getTouchedEntity instanceof Player){
            this.world.spawnEntity(new (Actor.getActor(this.getNBTsafe("entityID", Actor.FallStdEnemy.properties.id)).properties.class)(this.scale, JSON.parse(this.getNBTsafe("entityNBT", "{}"))), this.getNBTsafe("spawnX", this.getPosition.x), this.getNBTsafe("spawnY", this.getPosition.y+10));
            this.kill();
        }
    }
}
